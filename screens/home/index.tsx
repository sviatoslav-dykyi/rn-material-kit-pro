...
const Home = () => {
  ...
  const fetch = async () => {
    await fetchDossiers({ params, setDossiers, setIsLoading });
  };
  ...
  const handleDelete = (_id: string) => async () => {
    ...
    if ([200, 201].includes(response.status)) {
      fetchDossiers({ params, setDossiers, setIsLoading });
    }
  };
  ...
  return (
    <MenuProvider>
      <View style={styles.container}>
        ...
        ></Button>
        <ScrollView>
          <Block center style={[styles.imageBackground, { paddingTop: 20 }]}>
            {dossiers.map(
              (
                {
                  title,
